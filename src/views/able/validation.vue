<template>
  <el-card class="clo-center">
    ...
    <!-- <el-button type="primary">拼图验证</el-button> -->
...
const loadImage = () => {
  ...
  img.onload = () => {
    ...
    // sliderWidth.value = mainDom.width
    // sliderHeight.value = mainDom.height
    let r = 10 //半圆的半径
    let w = 60 //滑块的宽度
...
const changeValue1 = (val) => {
  ...
  let w = 60 //滑块的宽度
  var oldx = val.pageX
  let dom = document.getElementsByClassName('moveSlider')[0]
  dom.onmousemove = (e) => {
    var x = e.pageX
    if (sildLeft.value + x - oldx <= 0) {
      //这里判断左边界
      sildLeft.value = 1
      // } else if (sildLeft.value + x - oldx >= 280 - this.r * 2 - this.w) {
      //   //这里判断右边界
      //   sildLeft.value = 280 - this.r * 2 - this.w
      // } else {
      //   sildLeft.value += x - oldx
    }
    console.log(sildLeft.value)
    oldx = x
  }
  // leaveBall()
}
...
const leaveBall = () => {
  document.onmouseup = () => {
    ...
    if (
      sildLeft.value < sildX.val + value1.value &&
      sildLeft.value > sildX.val - value1.value
    ) {
      alert('恭喜你成功了')
    } else {
