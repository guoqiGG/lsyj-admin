<template>
  <el-card>
    <el-button type="primary" class="mt-4" @click="dialogVisibleHanle">编辑表格</el-button>
  </el-card>
  <el-card class="mt10">
  ...
  <el-dialog v-model="dialogVisible" title="编辑列表" width="80%" :before-close="handleClose">
    <el-table class="t1" row-key="id" ref="dragTable" :data="tableDialog" :row-class-name="tableRowClassName" :header-cell-style="{ backgroundColor: '#ecf5ff' }" border stripe style="width: 100%">
      <el-table-column v-for="item in DialogColumn" :key="item.props" :prop="item.props" :label="item.label">
        ...
        <template v-slot:default="scope" v-if="item.props === 'align'">
          <el-select v-model="scope.row[scope.column.property]" placeholder="Select">
            <el-option v-for="i in optionsType" :key="i.value" :label="i.label" :value="i.value" />
          </el-select>
        </template>
        ...
        <template v-slot:default="scope" v-if="item.props === 'fiexd'">
          <el-select v-model="scope.row.fixed" placeholder="Select">
            <el-option v-for="i in fiexdType" :key="i.value" :label="i.label" :value="i.value" />
          </el-select>
        </template>
      </el-table-column>
    </el-table>
